...
kontroll('belopp = 10 satser × 8 kartonger × 10 MB × 1.5 = 1200', ut[0].nyttolast.belopp === 1200);
...
p.onEvent(godis(528, 10), ctx);
kontroll('dubblett av samma sats ignoreras', ut.length === 1);
...
kontroll('efter prishöjning till 20: 5 × 8 × 20 × 1.5 = 1200', ut[1].nyttolast.belopp === 1200);

// 4. Ransonering halverar marginalen.
p.onEvent(godis(602, 5, { ransonerat: true }), ctx);
kontroll('ransonerat: halv marginal → 5 × 8 × 20 × 0.75 = 600', ut[2].nyttolast.belopp === 600);
...
kontroll('en insättning kan aldrig överstiga 5000', ut[3].nyttolast.belopp === 5000);

// 6. Bankens kvitto bokförs mot verifikatet.
p.onEvent({ id: 700, typ: 'kvitto', från: 'mybank', djup: 2, orsak: ut[0].id, nyttolast: { kvarter: 'svarmen-holding', belopp: 1200, saldo: 1300 } }, ctx);
...
kontroll('kvitto till oss bokförs', res.kvitterat === 1200);
kontroll('kvitto till moderbolaget bokförs INTE hos oss', res.kvitterat === 1200 && res.verifikat.filter(v => v.kvitto).length === 1);
kontroll('status visar bokfört = summan av insättningarna', res.bokfört === 1200 + 1200 + 600 + 5000);
...
kontroll('godis-klart från fel avsändare ger ingen insättning', ut.length === 4);
...
kontroll('spärrad insättning bokförs inte som intäkt', res2.bokfört === res.bokfört && res2.avstådda === 1);
neka = false;
